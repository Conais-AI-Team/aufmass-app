  ...
  try {
    ...
    if (leadsWithoutKN.rows.length > 0) {
      const branchPrefixMap = {
        ...
        'ayluxd': 'D', 'ayluxsi': 'SI', 'ayluxmu': 'MU', 'ayluxgk': 'GK', 'ayluxms': 'MS'
      };
...
async function seedLeadProductsFromConfig() {
  ...
  // Seed only active tenants. Retired/combined branch slugs must not be recreated
  // just because their historical catalog rows still exist.
